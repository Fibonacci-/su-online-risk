...
namespace ComputerDemo
{
    class RiskSequencer
    {
            ...
            else //active player
            {
                ...
                else if (client.state == MainState.Distribute)
                {
                    ...
                    else // move on to the next player's TradeCard phase
                    {
                        ...
                        //current = clients[(idx + 1) % clients.Count]; // go to the next player //TK. Shall we do this?
                    }
                }
        ...
        public void Run()
        {
            RiskMessage[] messages = new RiskMessage[clients.Count];
            for (int i = 0; i < clients.Count; ++i)
            {
                messages[i] = new RiskMessage(MainState.Start, clients[i].name);
            }
            ...
            while (bRunning)
            {
                ...
                for (int i = 0; i < clients.Count; ++i)
                {
                    //get the fresh map from the Server
                    {
                        ...
                        RiskMessage resp = this.Request(outgoing);
                    ...
                    //check for what to do
                    {
                        ...
                        RiskMessage resp = this.Request(outgoing); //Get what to do
                        if (resp.state != MainState.Idle)
                        {
                            ...
                            RiskMessage resp3 = this.Request(resp2); //update the Map at the Server side
                        }
                    }
                    if (this.gameOver(this.map))
                    {
                        bRunning = false;
                        break;
                    }
