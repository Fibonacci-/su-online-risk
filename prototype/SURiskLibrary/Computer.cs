...
//            Console.Out.WriteLine("     Computer::Update for " + nickname);
            foreach (ArmyPlacement p in incoming.territory_army)
            {
                ...
                selected.numArmies = p.numArmies; //TK
...
//              selected.numArmies += p.numArmies;
        ...
        public override RiskMessage Roll(RiskMessage incoming)
        {
            ...
            for (int i = 0; i < outgoing.roll.Length; i++)
            {
                ...
                //if (nickname == "Tom" && incoming.attacker)
                //{
                //    outgoing.roll[i] = 6;
                //}
            }
        ...
        {
            // if we have eliminated a player, we will have received their cards
            if (incoming.card.Count != 0)
            {
                foreach (ReinforcementCard c in incoming.card)
                {
                    ReinforcementCards.Add(c);
                }
            }
        ...
        public override RiskMessage ReinforcementCard(RiskMessage message)
        {
            ...
            {
                foreach(ReinforcementCard c in message.card)
                {
                    addCard(c);
                }                
            }
