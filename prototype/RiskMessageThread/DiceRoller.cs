...
namespace SURiskGUI
{
    public partial class DiceForm : Form
    {
        ...
        public DiceForm(Territory Attacking, int maxDice)
        {
            InitializeComponent();
            numberAttacker = null;  //new int[NumDiceAttacker];
            int[] dieChoiceAttacker = new int[maxDice];
            for (int i = 0; i < maxDice; ++i)
            {
                dieChoiceAttacker[i] = i + 1;
            }
            this.setOptionAttack(dieChoiceAttacker); //set the combobox's datasource.
            this.comboBox1.SelectedIndex = maxDice - 1; //as default, use as many dice as possible.
            //rg = new Random();
            setDieBlank();
            attackFrom = Attacking;
        }
        ...
        void RollAttacker3()
        {
            numberAttacker = new int[3];
            for (int i = 0; i < numberAttacker.Length; ++i)
            {
        ...
        public void setOptionAttack(int[] dieChoiceAttacker)//Player player)
        {
        ...
        private void buttonRoll_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (comboBox1.SelectedValue.Equals((object)NumDiceAttacker)) // && attackFrom.numArmies > 3)
                {
                ...
                else if (comboBox1.SelectedValue.Equals((object)(NumDiceAttacker - 1))) // && attackFrom.numArmies > 2)
                {
                ...
                else if (comboBox1.SelectedValue.Equals((object)(NumDiceAttacker - 2))) // && attackFrom.numArmies > 1)
                {
                    RollAttacker1();
                    pictureBox2.Hide();
                    pictureBox3.Hide();
                ...
                this.buttonRoll.Text = "OK";
                this.buttonRoll.DialogResult = System.Windows.Forms.DialogResult.OK;
            }
        }     
    }
}
