...
namespace ClientDemo2
{
    class LocalServer
    {
        List<LocalClient> clients = new List<LocalClient>();
        ...
        LocalClient current; //currently active client/player
        MainState state; //current state

        //these are used during the attack phase
        LocalClient attacker;
        LocalClient defender;
        ...
        public void Run()
        {
            ...
            while (incomingQueue.Count > 0)
            {
                ...
                {
                    LocalClient c = clients.Find(x => x.name == m.playerName);
                    c.Request(m);
                }
